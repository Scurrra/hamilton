...
#  - `data: Hamilton::Storage)` -- storage for some specific data
class Hamilton::Context
  private property default_method : Symbol | Nil
  private property inner : Hash(Int64 | String | Symbol, NamedTuple(method: Symbol | Nil, data: Hamilton::Storage))
  ...
  def initialize(*,
                 @inner = Hash(Int64 | String | Symbol, NamedTuple(method: Symbol | Nil, data: Hamilton::Storage)).new,
                 @default_method = nil)
  end
  ...
  def clean(key : Int64 | String | Symbol)
    if @inner.has_key?(key)
      @inner[key] = {method: @default_method, data: Hamilton::Storage.new}
      return @inner[key]
    end
    return nil
  end
  ...
  def clean_data(key : Int64 | String | Symbol)
    if inner = @inner[key]?
      @inner[key] = {method: inner[:method], data: Hamilton::Storage.new}
      return @inner[key]
    end
    return nil
  end
